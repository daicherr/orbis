...
export default function Home() {
  ...
  // Simula a criação do jogador quando o jogo começa
  useEffect(() => {
    const createPlayer = async () => {
      ...
      try {
        ...
        setPlayerId(data.id);
        setPlayerStats(data); // Armazena os status iniciais
        setMessages([{ type: 'narrator', text: `Bem-vindo, ${data.name}.` }]);
        
        // Pede a descrição inicial da cena
  ...
  useEffect(() => {
    // Verifica se deve entrar em modo de combate
  ...
  const handleSend = async (inputText, pId = playerId) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/game/turn?player_id=${pId}&player_input=${encodeURIComponent(inputText)}`, {
        method: 'POST',
      });
      ...
      setPlayerStats(data.player_state); // Atualiza os status do jogador
    ...
    <main className="flex min-h-screen flex-col items-center p-6 md:p-12 bg-cult-dark">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-7xl">
        ...
        <div className="md:col-span-2 flex flex-col">
          ...
          {inCombat ? (
            <CombatInterface skills={playerSkills} targets={npcsInScene.filter(n => n.emotional_state === 'hostile')} onAttack={handleAttack} />
          ) : (
